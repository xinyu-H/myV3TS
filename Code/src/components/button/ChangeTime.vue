...
onMounted(() => {
  ...
  const header = document.querySelector(".myPage header") as HTMLElement;
  const toggle = () => {
    is.value = !is.value;
    emits("onChange", is.value);
    header.style.backgroundImage = `linear-gradient(${
      is.value
        ? "to bottom right, rgb(87, 161, 235) 0%, rgb(151, 194, 236) 100%"
        : "to bottom right, rgb(46, 49, 53) 0%, rgb(74, 76, 78) 100%"
    }`;
    const isPressed = btn.matches("[aria-pressed=true]");
    btn.setAttribute(
      "aria-pressed",
      (isPressed ? false : true) as unknown as string
    );
  };
...
.pilot-bear {
  ...
  translate: calc((0 - (1 - var(--dark, 0))) * (var(--width) + 300%))
